...
-- Dumping data for table asti2.barang: ~4 rows (approximately)
...
CREATE TABLE IF NOT EXISTS `perolehan` (
  ...
  CONSTRAINT `FK_PEMASOK` FOREIGN KEY (`id_pemasok`) REFERENCES `asti`.`pemasok` (`id_pemasok`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

-- Dumping data for table asti2.perolehan: ~0 rows (approximately)
...
-- Dumping data for table asti2.ruang: ~2 rows (approximately)
